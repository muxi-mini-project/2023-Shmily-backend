...
func migration() {
	//自动迁移、创建数据库表
	DB.Set("gorm:table_options", "charset=utf8mb4").AutoMigrate(&User{}).AutoMigrate(&Memo{}).AutoMigrate(&Letter{}).AutoMigrate(&Anniversary{}).AutoMigrate(&AboutLover{}).AutoMigrate(&Friend{})
	...
	DB.Model(&Anniversary{}).AddForeignKey("Uid", "User(id)", "CASCADE", "CASCADE")
